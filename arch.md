...
- **Эмбеддинги**: Ollama HTTP API. Default deploy стартует без Ollama (`provider=none`). Для соседнего контейнера используется `http://ollama:11434` через compose overlay; для Ollama на macOS host используется Docker Desktop DNS `http://host.docker.internal:11434`. Модель по умолчанию `nomic-embed-text`.
...
│   │   └── tool_profile.go
│   └── admin/
│       ├── search.go            # внутрипроцессные debug-команды для `docker exec mcb mcb ...`
│       ├── add.go
│       ├── export.go
│       └── doctor.go
├── deploy/
...
# Логи идут в stdout, читаются через `docker logs mcb`
...
[embedding]
provider = "none"              # none | ollama; docker-compose.ollama.yml включает ollama
ollama_url = ""                # если provider=ollama: http://ollama:11434 или http://host.docker.internal:11434 на macOS
...
Внутри контейнера `mcb` — единственный бинарь, entrypoint по умолчанию `mcb serve`. Все остальные подкоманды вызываются через `docker exec mcb mcb <cmd>`.
...
```bash
docker exec mcb mcb search "auth middleware" --project /home/user/proj
docker exec mcb mcb sessions --limit 20
docker exec mcb mcb doctor
docker exec mcb mcb export > backup.jsonl
```
...
13. Если `mode == "manual"` → respond `204` (пользователь сам запустит `docker exec mcb mcb compact`).
...
- Распарсить body, залогировать в stdout (`docker logs mcb`): `{"event":"subagent_stop","session_id":"...","ts":...}`.
- respond `204`.
...
Если хочется ещё легче — `gcr.io/distroless/cc-debian12` runtime, образ ~20MB. Shell не нужен: Docker `HEALTHCHECK` вызывает сам бинарь (`mcb healthz`), а `docker exec mcb mcb search ...` работает напрямую. Дебажить distroless сложнее.
...
### Ollama на macOS host

Если Ollama уже запущена на macOS host, не использовать `docker-compose.ollama.yml`. Docker Desktop даёт контейнеру DNS имя `host.docker.internal`, поэтому достаточно local override file:
...
services:
  mcb:
    environment:
      MCB_EMBEDDING_PROVIDER: "ollama"
      MCB_EMBEDDING_OLLAMA_URL: "http://host.docker.internal:11434"
```
...
```bash
# Backup
docker exec mcb mcb backup --out - > ./mcb-$(date +%Y%m%d).db
...
- README: install, troubleshooting (`curl: (7)` → `docker ps`/`docker logs mcb`, `SQLITE_BUSY` → pool/busy timeout, Ollama timeout → `mcb doctor`/circuit breaker state, settings merge → `jq` recipe), backup/restore, no migration rollback guarantee
...
- `memory_update` для правки `tier`/`importance`/text без создания новой memory, если save+supersede окажется неудобным
...
- [ ] `docker exec mcb mcb add "fact" --project p1` создаёт memory, `docker exec mcb mcb search "fact" --project p1` находит
